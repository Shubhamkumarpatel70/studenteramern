...
const AdminSidebar = ({ isSidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 bg-white shadow-2xl w-72 transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out md:relative md:translate-x-0 z-50 border-r border-gray-100`}
    >
      ...
      <nav className="flex flex-col p-4 space-y-6 overflow-y-auto h-full pb-20">
        ...
        <div className="p-4 mt-6 pt-6 border-t border-gray-100">
          <NavLink
            to="/logout"
            className={({ isActive }) =>
              `${commonClasses} ${isActive ? activeClass : inactiveClass}`
