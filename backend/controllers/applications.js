// @desc    Process refund for a rejected application (Admin)
// @route   PUT /api/applications/:id/refund
  ...
  try {
    ...
    if (application.status !== "Rejected") {
      return res.status(400).json({
        success: false,
        message: "Refunds can only be processed for rejected applications.",
      });
    }
    application.refundUtr = refundUtr;
    application.refundStatus = "Completed";
    application.refundDate = new Date();
    ...
    if (application.user && application.user._id) {
      await createNotification(
        ...
        `Your refund of ₹${application.amount || 149} for "${application.internship?.title || "your application"}" has been initiated. Refund UTR: ${refundUtr}`,
      );
    }
...
exports.createApplication = async (req, res, next) => {
  try {
    const { internshipId, duration, certificateName, utr, paymentScreenshot } =
      req.body;
    const userId = req.user.id;

    console.log(
      "Creating application with paymentScreenshot:",
      paymentScreenshot,
    );
    ...
    if (internship.currentRegistrations >= internship.totalPositions) {
      ...
      return res.status(400).json({
        success: false,
        message: "All positions for this internship are filled.",
      });
    }
    ...
    res
      .status(400)
      .json({ success: false, message: "Could not create application" });
  }
};
...
exports.getApplicationById = async (req, res, next) => {
  try {
    const application = await Application.findById(req.params.id)
      ...
      .populate(
        "internship",
        "title company image stipend location duration technologies description shortDescription",
      );
...
exports.updateApplicationStatus = async (req, res, next) => {
  try {
    ...
    // Only set transactionId and create Transaction if approved
    if (status === "Approved") {
      // Set transactionId if not already set
      if (!application.transactionId) {
        application.transactionId = `SE_txn_${Date.now()}`;
      }
      await application.save();
      ...
      if (!existingTxn) {
        await Transaction.create({
          ...
          amount: application.amount || 149,
          purpose: `Internship Application: ${application.internship?.title || "N/A"}`,
          status: "Completed",
          transactionId: application.transactionId,
        });
      }
    ...
    // Create Transaction if payment received and not already exists
    if (paymentReceived) {
      const Transaction = require("../models/Transaction");
      const existingTxn = await Transaction.findOne({
        transactionId: application.transactionId,
      });
      if (!existingTxn && application.transactionId) {
        await Transaction.create({
          user: application.user,
          amount: application.amount || 149,
          purpose: `Internship Application: ${application.internship}`,
          status: "Completed",
          transactionId: application.transactionId,
        });
      }
      ...
      .upload_stream(
        ...
        async (error, result) => {
          ...
          if (req.body.applicationId) {
            ...
            await application.save();
            console.log(
              "Payment screenshot uploaded and application updated:",
              result.secure_url,
            );
          }
