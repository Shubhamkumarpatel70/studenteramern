...
exports.createMeeting = async (req, res, next) => {
  ...
  try {
    ...
    if (
      req.body.targetType === "users" &&
      (!req.body.selectedUsers || req.body.selectedUsers.length === 0)
    ) {
      return res
        .status(400)
        .json({
          success: false,
          message: "Please select at least one user for targeted meetings.",
        });
    }
    if (req.body.targetType === "internship" && !req.body.selectedInternship) {
      return res
        ...
        .json({
          success: false,
          message: "Please select an internship for targeted meetings.",
        });
    }
    if (
      req.body.targetType === "co-admins" ||
      req.body.targetType === "accountants"
    ) {
    ...
    res
      .status(400)
      .json({ success: false, message: "Could not create meeting" });
  }
};
...
exports.updateMeeting = async (req, res, next) => {
  try {
    ...
    res
      .status(400)
      .json({ success: false, message: "Could not update meeting" });
  }
};
